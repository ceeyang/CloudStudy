...
class LoginView: UIView {
    ...
    func setupUI() {
        ...
        loginBtn.snp.makeConstraints { (make) in
            make.top.equalTo(inputAreaBackgroundView.snp.bottom).offset(15)
            make.left.equalTo(inputAreaBackgroundView.snp.left).offset(10)
            make.right.equalTo(inputAreaBackgroundView.snp.right).offset(-10)
            make.height.equalTo(height)
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if (siteText?.isFirstResponder)! {
            siteText?.resignFirstResponder()
        }
        if (accountText?.isFirstResponder)! {
            accountText?.resignFirstResponder()
        }
        if (paswordText?.isFirstResponder)! {
            paswordText?.resignFirstResponder()
        }
    }
